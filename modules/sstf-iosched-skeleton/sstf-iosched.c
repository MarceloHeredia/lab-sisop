...
static int sstf_dispatch(struct request_queue *q, int force){
	...
	struct list_head *chosen = NULL;
	rq = list_first_entry_or_null(&nd->queue, struct request, queuelist);

	if(rq){

		int distance = abs(blk_rq_pos(rq) - nd->head);

		list_for_each(chosen, &nd->queue)
		{
			struct request *le = list_entry(chosen, struct request, queuelist);
			int dist_this = abs(blk_rq_pos(le) - nd->head);
			if (dist_this < distance){
				rq = list_entry(chosen, struct request, queuelist);
				distance = dist_this;
			}
		}
		...
		nd->head = blk_rq_pos(rq);
		//printk(KERN_EMERG "[SSTF] dsp %c %llu sec:%llu\n", direction, blk_rq_pos(rq), nd->head);
		printk("Dsp %llu", blk_rq_pos(rq));
		return 1;
	}
